...
func TestHearing_Markdown(t *testing.T) {
	tests := []struct {
		...
		{name: "empty", hearing: Hearing{}, want: ""},
		{
			name: "single topic",
			hearing: Hearing{
				Time:  []time.Time{time.Date(2022, time.March, 29, 11, 0, 0, 0, time.Local)},
				Place: []string{"г.Брянск, ул. Клинцовская, д. 60 (здание Городского Дворца культуры им. Д.Н. Медведева)"},
				Topic: []string{
		...
		},
		{
			name: "multi topics",
			hearing: Hearing{
				Time:  []time.Time{time.Date(2021, time.March, 17, 11, 0, 0, 0, time.Local)},
				Place: []string{"ГДК Советского района (ул. Калинина, д. 66)"},
				Topic: []string{
